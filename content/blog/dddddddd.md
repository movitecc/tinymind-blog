...
<body>
    ...
    <script>
        ...
        // 创建网格项
        function createGridItem(value, itemClass) {
            const gridItem = document.createElement('div');
            gridItem.className = `grid-item ${itemClass}`;
        ...
        function fillGrid(gridId, results, itemClass, emptyMessage = '没有数据') {
            ...
            // 如果结果不足10个，填充空白项以保持网格结构
            if (displayResults.length < 10) {
                for (let i = 0; i < 10 - displayResults.length; i++) {
                    const emptyItem = document.createElement('div');
